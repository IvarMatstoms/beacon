    ...
    try:
        ...
        try:
            ...
            if userInput.startswith(">>>"):
                prepend_output("run with python","gnome-terminal -e \"bash -c \\\" python -c \\\\\\\\\\\"%s \\\\\\\\\\\" && read && exit \\\"\"" % (userInput[3:]))
            #f=open("/home/ivar/llog","w")
            #f.write()
            #f.close()
            #prepend_output("run with python","gnome-terminal -e \"python -c \\\"print ('hi') \\\"\"" % (userInput))
                #prepend_output("python: "+str(out),"terminator -e python2.7 -i -c 'print "+userInput+"'")
        except Exception as e:
            print (e)
            print ("err")
        try:
            if userInput.startswith("calc "):
                prepend_output(userInput[5:]+"="+str(eval(userInput[5:])),"")
        except Exception as e:
            pass
